...
def _apply_light_input_palette(widget):
    """اعمال کنتراست مستقل از تم سیستم‌عامل روی تمام ورودی‌های متنی."""
    current = widget.palette()
    palette = QPalette(current)
    active_groups = (QPalette.Active, QPalette.Inactive)
    for group in active_groups:
        palette.setColor(group, QPalette.Base, QColor("#ffffff"))
        palette.setColor(group, QPalette.AlternateBase, QColor("#f7f8fa"))
        palette.setColor(group, QPalette.Text, QColor("#111827"))
        palette.setColor(group, QPalette.WindowText, QColor("#111827"))
        palette.setColor(group, QPalette.ButtonText, QColor("#111827"))
        palette.setColor(group, QPalette.Highlight, QColor("#245fa8"))
        palette.setColor(group, QPalette.HighlightedText, QColor("#ffffff"))
        if hasattr(QPalette, "PlaceholderText"):
            palette.setColor(group, QPalette.PlaceholderText, QColor("#6b7280"))
    palette.setColor(QPalette.Disabled, QPalette.Base, QColor("#edf0f3"))
    palette.setColor(QPalette.Disabled, QPalette.Text, QColor("#4b5563"))
    palette.setColor(QPalette.Disabled, QPalette.WindowText, QColor("#4b5563"))
    palette.setColor(QPalette.Disabled, QPalette.ButtonText, QColor("#4b5563"))
    if hasattr(QPalette, "PlaceholderText"):
        palette.setColor(QPalette.Disabled, QPalette.PlaceholderText, QColor("#7b8491"))
    if palette == current:
        return False
...
def _polish_input(widget):
    if not _qobject_is_alive(widget):
        return
    # جلوگیری از حلقه PaletteChange <-> style.polish در macOS که می‌تواند
    # رشته اصلی رابط را اشغال و فرم ورود را کاملاً غیرقابل‌پاسخ کند.
    if widget.property("_inputPolishBusy") is True:
        return
    try:
        widget.setProperty("_inputPolishBusy", True)
        property_changed = False
        if widget.property("cleanInput") is not True:
            widget.setProperty("cleanInput", True)
            property_changed = True
        # همه فیلدهای ادمین سطح روشن و متن تیره دارند.
        if widget.property("lightInputSurface") is not True:
            widget.setProperty("lightInputSurface", True)
            property_changed = True
        palette_changed = _apply_light_input_palette(widget)
        # setPalette خودش رابط را به‌روزرسانی می‌کند. unpolish/polish دوباره در
        # macOS می‌تواند PaletteChange جدید بسازد و یک حلقه بی‌پایان ایجاد کند.
        if property_changed or palette_changed:
            widget.update()
    except (RuntimeError, ReferenceError):
        return
    finally:
        if _qobject_is_alive(widget):
            try:
                widget.setProperty("_inputPolishBusy", False)
            except (RuntimeError, ReferenceError):
                pass
    ...
    def eventFilter(self, obj, event):
        try:
            ...
            if (
                ...
                isinstance(obj, (QLineEdit, QComboBox, QTextEdit, QPlainTextEdit, QAbstractSpinBox))
                # PaletteChange عمداً حذف شده است؛ تغییر پالت داخل _polish_input
                # در macOS می‌توانست دوباره همین رویداد را تولید و رابط را قفل کند.
                and event.type() in (QEvent.Show, QEvent.EnabledChange)
            ):
                QTimer.singleShot(0, lambda target=obj: _safe_invoke(_polish_input, target))
        except (RuntimeError, ReferenceError):
            pass
        except Exception:
            pass
